...
def welcome():
    ...
    """
    # change the terminal titile to 'Hotel Reviews'
    
    """Display a welcome message with the title 'Hotel Reviews'."""
    title = 'Hotel Reviews'
    
    # Create the formatted message with asterisks
    formatted_message = '\n' + '*' * (len(title)) + ' '
    formatted_message +=  title + ' '
    formatted_message += '*' * (len(title) ) + '\n'
...
def error(msg):
    ...
    :return: does not return anything
    """
    """Display an error message based on the provided msg parameter."""
    error_msg = msg
...
def progress(operation, value):
    ...
    :return: does not return anything
    """
    """Display a message to indicate the progress of an operation."""
    print(f"Operation: {operation} [{value}]. \r", end="")
...
def main_menu():
    ...
    """Display a menu of options and retrieve the user's choice."""
    options = [
        "Process Data",
        "Visualise Data",
        "Export Data",
...
def sub_menu(variant=0):
    ...
    :param variant: Determines which set of menu options to display.
    :return: 0 if invalid selection otherwise an integer for a valid selection.
    """
    if variant == 0:
        print("Error: No valid menu variant provided.")
        return 0
    
    # Define the menu options for each variant
    menu_options = {
        1: ['[1] Reviews for Hotel', '[2] Reviews for Dates', '[3] Reviews for Nationality', '[4] Reviews Summary'],
        2: ['[1] Positive/Negative Pie Chart', '[2] Reviews Per Nationality Chart', '[3] Animated Summary'],
        3: ['[1] All Reviews', '[2] Reviews for Specific Hotel']
    }
    
    # Display the appropriate menu based on the variant
    if variant in menu_options:
        print("\nSub Menu:")
        for option in menu_options[variant]:
            print(option)
        ...
        while True:
            try:
                ...
                if 1 <= choice <= len(menu_options[variant]):
                    return choice
                else:
        ...
        print("Error: Invalid menu variant provided.")
        return 0
...
def hotel_name():
    ...
    :return: the name of a hotel
    """
    # Prompt the user for a hotel name and return it
    return input("Please enter a hotel name (e.g. Hotel Arena): ")
...
def review_dates():
    ...
    while True:
        date_str = input("Enter a review date (mm/dd/yyyy) or press Enter 'q' to finish: ")
        if date_str == "" or date_str.lower() == "q":
            break  # Exit the loop if Enter is pressed

        try:
            # Parse the input date string into a datetime object
            date_obj = datetime.datetime.strptime(date_str, "%m/%d/%Y")
            # Convert the datetime object back to the specified format and add it to the list
            formatted_date = date_obj.strftime("%m/%d/%Y")
            review_dates.append(formatted_date)
        except ValueError:
            error("Invalid date format. Please enter a date in the format mm/dd/yyyy.")
...
def display_review(review, cols=None):
    ...
    if not cols:
        displayed_data = review
    else:
        displayed_data = [review[i] for i in cols]
...
def display_reviews(reviews, cols=None):
    ...
    for review in reviews:
        # Determine which columns to display for the current review
        if not cols:
            displayed_data = review
        else:
            displayed_data = [review[i] for i in cols]
        
        # Display the selected data for the current review
        print(displayed_data)
...
def file_name():
    return input("Enter the filename for the JSON export (e.g. temp.json): ")